...
angular
		...
		.controller(
				...
				function($scope, $http, leafletData, $filter, AlertService,
						MAPSERVER, MAXS2LEVEL, leafletEvents, WEBSERVICE,
						$interval) {
					...
					function getProviders() {
						$http
								.get(WEBSERVICE + '/measurementproviders', {})
								.error(function(err) {
									console.log(err);
								})
								.success(
...
//																	});
															$http
																	.get( WEBSERVICE + '/provider',{ params:{ _id:provider.key}})
																	.error(	function(err) {
																		console.log(err);
																	})
					...
					function getGeoHashes() {
						...
						if ($scope.bounds) {
							...
							var params = {
								datePrecision : 730,
								geohashPrecision : (gp),
								bottom_left : [ $scope.bounds.southWest.lat,
										$scope.bounds.southWest.lng ],
								top_right : [ $scope.bounds.northEast.lat,
					...
					$scope.$watch('center.zoom', function(newVal, oldVal) {
						...
						// if($scope.s2zoom>=MAXS2LEVEL){$scope.s2zoom=MAXS2LEVEL.toString()+"
						// (max)";}
					...
					function onEachFeature(feature, layer) {
						var popup = '<strong> Metingen: </strong> '
								+ feature.count
								+ "<br>"
								// + '<strong> Eerste meting: </strong> ' +
								// $filter('date')(feature.firstseen,'dd-MM-yyyy
								// hh:mm:ss')+ "<br>"
								+ '<strong> Laatste meting: </strong> '
								+ $filter('date')(feature.lastseen,
					...
					angular.extend($scope, {
						...
						events : {
							...
							// },
							marker : {
								disable : leafletEvents
										.getAvailableMarkerEvents()
							},
							path : {
								disable : leafletEvents
										.getAvailablePathEvents()
							}
						}
					});
