...
angular.module('celllogger')
  	...
  	.controller('CellsCtrl', function ($scope,$http,leafletData,$filter,AlertService,MAPSERVER,$q,WEBSERVICE,geohash) {

  		$scope.desaturate=true;
  		var markers={};
  		$scope.cells=[];
  		$scope.search={geohashPrecision:7,serving:null};
		...
		// var colorFunction = new L.HSLHueFunction(new L.Point(1,130), new L.Point(500,0), {outputSaturation: '100%', outputLuminosity: '0%'});
		// var fillColorFunction = new L.HSLHueFunction(new L.Point(1,130), new L.Point(500,0), {outputSaturation: '100%', outputLuminosity: '50%'});
		...
		function centerOnHash(geoHash,zoom){
  		 ...
  		 $scope.$watch('search',function(newVal,oldVal){
  		 		if(newVal.geohashPrecision!==oldVal.geohashPrecision || newVal.serving!==oldVal.serving){
  				...
  				{
	  				if($scope.search.cid || $scope.search.mnc || $scope.search.lac || $scope.search.mnc){
	  					getCell();
	  				}
  				}
	  ...
	  //10870
	  	$scope.paintCellCoverage=function(hit){
		        ...
		        .success(function(result){
		        	...
		        	if(result.aggregations.cellgrid.buckets.length>0){
		        		...
		        		result.aggregations.cellgrid.buckets.forEach(function(grid){
	  	...
	  	function createMarker(hit)
	  	{
			...
			var marker={
				...
				riseOnHover:true,
				draggable:false,
				label:hit.mcc + "-" + hit.net + "-" + hit.area + "-" + hit.cell,
				title:hit.mcc + "-" + hit.net + "-" + hit.area + "-" + hit.cell,
				source:hit.source,
				// signal:hit.signal,
				message:'<strong>Provider: </strong>'+hit.net +' (' + hit.provider +')<br>'+
					'<strong>Network: </strong>'+hit.radio +'<br>'+
					'<strong>Source: </strong>'+hit.source +'<br>'+
					'<strong>Date: </strong>'+$filter('date')(hit.timestamp,'dd-MM-yyyy') +'<br>'+
					'<strong>Azimuth: </strong>'+ hit.azimuth +'<br>'+
					'<strong>Beam Width: </strong>'+ hit.beamwidth +'<br>'
	 ...
	 //  				ret.push({mcc:res._id.mcc,mnc:res._id.mnc,lac:res._id.lac,cid:res._id.cid,signal:res.signal,network:res.network,lat:res.location[1],lon:res.location[0]});
		// 			// if(res.location){
		// 			// 	console.log(res);
		// 			// 	var key=res._id.mcc.toString()+res._id.mnc.toString()+res._id.cid.toString()+res._id.lac.toString();
		// 			// 	res.key=key;
		// 			// 	var marker={
		// 			// 		lat:res.location[1],
		// 			// 		lng:res.location[0],
		// 			// 		draggable:false,
		// 			// 		label:res._id.mcc + "-" + res._id.mnc + "-" + res._id.lac+ "-" + res._id.cid + " " + res.network,
		// 			// 		title:res._id.mcc + "-" + res._id.mnc + "-" + res._id.lac+ "-" + res._id.cid + " " + res.network,
		// 			// 		signal:res.signal,
		// 			// 		message:'<strong>Cell-id: </strong>'+res._id.cid +'<br>'+
		// 			// 			'<strong>Provider: </strong>'+res._id.mnc +' (' + lookupMNC(res._id.mcc,res._id.mnc) +')<br>'+
  ...
  // 			params.center=$scope.center;
		...
		// 	var key=hit.mcc.toString()+hit.mnc.toString()+hit.lac.toString()+hit.cid.toString();
		// 	var markerKey=hit.mcc.toString()+hit.mnc.toString()+hit.cid.toString()+hit.lac.toString()+hit.timestamp.toString();
