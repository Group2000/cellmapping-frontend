...
angular.module("leaflet-directive").directive('geohash', ["$log", "$rootScope", "leafletData", "leafletHelpers", function ($log, $rootScope, leafletData, leafletHelpers) {
    return {
        restrict: "A",
        scope: false,
        replace: false,
        ...
        link: function(scope, element, attrs, controller) {
            ...
            controller.getMap().then(function(map) {
                leafletScope.$watch("geohash", function(geohash) {
                    
                    if (isDefined(leafletGeoHash) && map.hasLayer(leafletGeoHash)) {
                        map.removeLayer(leafletGeoHash);
                    }

                    if (!(isDefined(geohash) && isDefined(geohash.data))) {
                        return;
                    }

                    geohash.options
                        leafletGeoHash = new L.GeohashDataLayer(geohash.data,geohash.options);
                        leafletData.setGeoHash(leafletGeoHash, attrs.id);
                        map.addLayer(leafletGeoHash);
...
angular.module("leaflet-directive").directive('cellgeohash', ["$log", "$rootScope", "leafletData", "leafletHelpers", function ($log, $rootScope, leafletData, leafletHelpers) {
    return {
        restrict: "A",
        scope: false,
        replace: false,
        ...
        link: function(scope, element, attrs, controller) {
            ...
            controller.getMap().then(function(map) {
                leafletScope.$watch("cellgeohash", function(geohash) {
            
                    if (isDefined(leafletCellGeoHash) && map.hasLayer(leafletCellGeoHash)) {
                        map.removeLayer(leafletCellGeoHash);
                    }
                    ...
                    geohash.options
                        leafletCellGeoHash = new L.GeohashDataLayer(geohash.data,geohash.options);
                        leafletData.setGeoHash(leafletCellGeoHash, attrs.id);
                        map.addLayer(leafletCellGeoHash);
            ...
            mapController.getMap().then(function(map) {
                ...
                getLayers().then(function(layers) {
                    ...
                    leafletScope.$watch('markers', function(newMarkers) {
                        // console.log('$watch for markers hit');
        ...
        link: function(scope, element, attrs, controller) {
            ...
            controller.getMap().then(function(map) {
                if (isDefined(L.Control.Draw) && isDefined(controls.draw)) {
                    ...
                    angular.extend(options, controls);
                ...
                // //ADDED HG
                if (isDefined(L.Control.Fullscreen) && isDefined(controls.fullscreen)) {
                  var fullscreenControl = new L.Control.Fullscreen(controls.fullscreen.options);
...
angular.module("leaflet-directive").service('leafletData', ["$log", "$q", "leafletHelpers", function ($log, $q, leafletHelpers) {
    ...
    this.setGeoHash = function(leafletGeoHash, scopeId) {
        var defer = getUnresolvedDefer(geoHash, scopeId);
        defer.resolve(leafletGeoHash);
        setResolvedDefer(geoHash, scopeId);
    };

    this.getGeoHash = function(scopeId) {
        var defer = getDefer(geoHash, scopeId);
        return defer.promise;
    };

    this.setCellGeoHash = function(leafletGeoHash, scopeId) {
        var defer = getUnresolvedDefer(cellGeoHash, scopeId);
        defer.resolve(leafletGeoHash);
        setResolvedDefer(cellGeoHash, scopeId);
    };

    this.getCellGeoHash = function(scopeId) {
        var defer = getDefer(cellGeoHash, scopeId);
        return defer.promise;
    };
...
angular.module("leaflet-directive").factory('leafletLayerHelpers', ["$rootScope", "$log", "leafletHelpers", function ($rootScope, $log, leafletHelpers) {
    ...
    var layerTypes = {
        ...
        //             return;
        //         }
        //         return new L.GeohashDataLayer(geohashData,options);
    ...
    return {
        ...
        addMarkerWatcher: function(marker, name, leafletScope, layers, map) {
            var clearWatch = leafletScope.$watch("markers[\""+name+"\"]", function(markerData, oldMarkerData) {
                ...
                //ADDED HG
                if (isDefined(markerData.opacity)) {
...
angular.module("leaflet-directive").factory('leafletHelpers', ["$q", "$log", function ($q, $log) {
    ...
    return {
        ...
        //     is: function(layer) {
        //         if (this.isLoaded()) {
        //             return layer instanceof L.HeatLayer;
        //         } else {
        //             return false;
        //         }
        //     }
